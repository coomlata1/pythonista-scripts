...
#---Author: @coomlata1
#---Created: 01/28/2015
#---Last Updated: 07/04/2016-Rewrote code to support the upgrade of Pythonista to version 2.1 & the accompanying changes to the 'Photo' module. Requires Pythonista 2.1 or greater.

This Pythonista 2.1 script and it's 2 pyui files, 'PhotosTo Dropbox.pyui' and 'PhotosToScale.pyui', will RESIZE, RENAME, GEO-TAG & UPLOAD all selected photos in the iPhone camera roll to new folders in your Dropbox account. The main folder will be named after the year the photo was taken in the format 'yyyy', & the subfolders will be named for the date the photo was created in the format mm.dd.yyyy. The photos themselves will have the exact time the photo was taken amended to the front of their names in the format hh.mm.ss.XXXX.jpg, where XXXX is the original name. All metadata in the original photo will be copied to the resized & renamed copy in Dropbox if desired. The script allows you to select your desired photo scaling options.

This script requires that the pexif module, available at https://github.com bennoleslie/pexif, be imported into Pythonista. Just copy pexif.py into the Pythonista 'site-packages' dir. Pexif allows for both reading from and writing to the metadata of image files. Many thanks to Ben Leslie for maintaining the pexif module at github.

Script also requires DropboxLogin.py, available at https://gist.github.com/omz/4034526, which allows login access to Dropbox.

The pyui interface dimensions were set up for an iPhone and will need to be tweaked slightly to look good on an iPad. 

Many thanks to @cclauss for excellent help and advice with tightening the code and improving the program flow of the script.
'''
import console
import location
import photos
  ...
  for asset in assets:
    ...
    if geo_tag:
      ...
      if the_location:
        ...
        '''
        Get degrees needed to rotate photo for it's proper orientation. See www.impulsesdventue.com/photoexif-orientation.html for more details.
        '''
