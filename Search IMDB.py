...
This Pythonista script uses the api available
at www.omdbapi.com to search for your desired
movie, TV series, or actor-actress
information.

The movie-tv query will first return the most
popular result for what matches that title.
The script will display most of the pertinent
info in a ui.TextView screen about that movie
or TV series including release date, imdb
rating, box office returns, writers,
director, actors and plot.

That may not be the results you are looking
for. You can then refine your search and the
query will return a list of titles and their
release year that match or are close to
matching the desired title. When you select a
title from the list you will get a new set of
results for that title. You can keep refining
your search until you get the results you are
looking for. The info is displayed in a ui
TextView screen and formatted for Markdown.

There is also an option button available
which displays the query results directly
from the imdb web site on a WebView screen.

The actor-actress query results will display
on a Webview page directly from the imdb
website. The results here are for viewing
only. There is no option provided in the
script to export the results.

You can export the results of the movie-tv
queries to 1Writer, DayOne, Drafts, Editorial
or the clipboard. The results can also be
imported to these apps if this script is
called via a url from the app itself.

When exported to a Markdown capable editor,
the movie title & poster appear in hypertext
with a direct link to the IMDB database if
more info is desired.

This script is also capable of displaying the
query results in markdown TextView as a
result of MarkdownView.py, a script placed in
the site-packages folder, and called as a
module in this script.

Requirements: Markdown.py which is available
at: https://github.com/mikaelho/pythonista-
markdownview. Be sure to read the readme.md
file for installation instructions.

Optional: The tmdb api at https://
www.themoviedb.or/en provides a more detailed
overview of cast, crew, and plot. To use the
api you will need to register, whereupon you
will receive an api key.

A pure python wrapper for the tmdb api,
'tmdbsimple', allows for an easy way to code
your tmdb api queries. It is available at
https://pypi.python.org/pypi/tmdbsimple. I
installed it into Pythonista using Stash by
typing 'pip install tmdbsimple' at the Stash
command line. Further instructions and
information are available on the tmdbsimple
webpage.
'''
...
class MyView(ui.View):
  ...
  def query(self, sender):
    ...
    if self.sc.selected_index == 0:
      ...
      popular result for query, but not
      always the one you desire when there
      are multiple titles that are the same
      or similar to the title you are looking
      for.
      '''
      # Call subroutines
  ...
  def refine(self, sender):
    ...
    rd = query_data(url)
    '''
    Call function to list all the titles in
    the query & a list of their respective
    IMDB ids.
    '''
...
movie info & return a text of those results
to the caller for display in a TextView.
'''
def mine_console_data(d):
  ...
  '''
  If you have installed the tmdbsimple python
  module and you have a tmdb api key then you
  can mine more data.
  '''
...
'''
Function to mine query results for desired
movie info & return a Markdown text of those
results for copying to the clipboard.
'''
def mine_md_data(d):
...
'''
Function that returns a list of multiple
titles that contain the film-tv name if not
satisfied with results of 1st query, and a
...
def list_data(d):
  ...
  '''
  Loop through query results & add the year,
  title, type, and IMDB id of every media but
  'episodes' to film-tv set.
  '''
...
def main():
  ...
  '''
  If view was closed with the 'X' on title
  bar & script was called from another app,
  then return to calling app & exit script,
  otherwise just cancel script.
  '''
  #if app:
    #if app == '1Writer':
      #app = 'onewriter'
